...
Author: Logan Wilson
https://github.com/lcwilson95/MusicMaker/
...
To install PIL, running "pip install Pillow" within the Terminal / Command Prompt
(recommended) or download from http://python-pillow.org/.
Pydub, treble clef.png, arial.ttf, as well as folders containing Piano Samples,
Major Chords, and Minor Chords, are all required (included with download).

After running MusicMaker.py, sheet music .jpg are generated within the
Sheet Music folder, and audio .wav files are generated within the Audio Files folder.
...
#Need PIL in order to run Treble class for generating sheet music.
#In Terminal / Command Prompt, run "pip install Pillow"
...
class Treble(object) :
    ...
    def __init__(self,key) :
        ...
        #Key signature
        font = ImageFont.truetype('arial.ttf', 18) #Need arial.ttf file in working directory
        if key == "G" :
    ...
    def save(self,filename) :
        ...
        self._image.save(os.getcwd()+"/Sheet Music/"+filename+".jpg")
...
class Sinatra(object) :
    ...
    def export(self,file,filename) :
        ...
        file.export(os.getcwd()+"/Audio Files/"+filename+".wav", format="wav")
...
def main() :
    ...
    mA = Mozart()
    mA.choose_key("G")
    mA.choose_progression(["I","V","vi","iii","IV","I","IV","V"])
